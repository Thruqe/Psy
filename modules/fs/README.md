...
START
    ...
    IF EXISTS("modules") AND ISDIR("modules") THEN
        ...
        fsExists = EXISTS("modules/fs")
        IF fsExists AND ISDIR("modules/fs") THEN
        ...
        mathExists = EXISTS("modules/math")
        IF mathExists AND ISDIR("modules/math") THEN
...
1. Create a new file in `modules/fs/src/` (e.g., `copy_file.rs`)
2. Implement the function with signature: `pub fn copy_file(args: &[Value]) -> Result<Value, String>`
3. Register the function in `modules/fs/src/lib.rs`
